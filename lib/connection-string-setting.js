...
ConnectionStringSetting.prototype.explodeConnectionString = function (connectionString) {
    const parts = this.originalConnectionString.split(";");
    return parts;
};

ConnectionStringSetting.prototype.getConnectionString = function (timeoutValueInMilliseconds = 5000) {
    ...
    _.each(parts, part => {
        ...
        if(trimmed.toLowerCase().startsWith("connection timeout"))
            return;
